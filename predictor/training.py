...
class CustomLoss(torch.nn.Module):
    ...
    def forward(self, preds, targets):
        ...
        loss_goals = F.mse_loss(preds["goals_scored"], xgTarget)
        loss_assists = F.mse_loss(preds["assists"], xaTarget)
        loss_cs = focal_loss(
        ...
        total = self.weights["xg"]*loss_goals + self.weights["xa"]*loss_assists + self.weights["cs"]*loss_cs + self.weights["mins"]*loss_will_play + self.weights["mins"]*loss_p_60
        return total, {
            "loss_xg": loss_goals.detach(),
            "loss_xa": loss_assists.detach(),
